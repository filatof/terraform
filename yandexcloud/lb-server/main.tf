#создает одну ВМ и loadbalanser
terraform {
  required_providers {
    yandex = {
...
# целевая группа инстансов
resource "yandex_lb_target_group" "test" {
  name      = "my-target-group"
  region_id = "ru-central1"
  # в примере состоит из одного инстанса
  target {
    subnet_id = yandex_vpc_subnet.test-subnet.id
    address   = yandex_compute_instance.vm-test.network_interface.0.ip_address
  }
}

#network loadbalanser 
resource "yandex_lb_network_load_balancer" "web" {
  name = "load-balancer-for-test"
...
output "web_loadbalancer_url" {
  value = [for listener in yandex_lb_network_load_balancer.web.listener : [for addr in listener.external_address_spec : addr.address if listener.name == "my-listener"][0]][0]
}
